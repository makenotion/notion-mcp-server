...
export class MCPProxy {
  ...
  private readonly READ_ONLY_OPERATIONS = new Set([
    "post-search",
    "query-data-source",
  ]);
  ...
  private setupHandlers() {
    ...
    this.server.setRequestHandler(ListToolsRequestSchema, async () => {
      ...
      Object.entries(this.tools).forEach(([toolName, def]) => {
        def.methods.forEach((method) => {
          ...
          // Treat POST-based read operations as read-only instead of inferring
          // annotations solely from the HTTP method.
          const operation = this.openApiLookup[toolNameWithMethod];
          const httpMethod = operation?.method?.toLowerCase();

          const isReadOnly =
            httpMethod === "get" || this.READ_ONLY_OPERATIONS.has(method.name);
    ...
    this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
      ...
      const deserializedParams = params
        ? deserializeParams(params as Record<string, unknown>)
        : {};
